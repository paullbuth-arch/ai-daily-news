...
[[WQ Audio Protocol]] SDK 已经有 4 类 Service (工具/音频下行/音频上行/录制)，但 reGlasses 项目需要**再加 4 类**：设备控制、配置管理、遥测上报、IMU 数据流。这里定义了每一类里有哪些命令、每个命令的 payload 长什么样。
...
- [[WQ Audio Protocol]] — 帧协议基础
- [[WQ Protocol 服务类型]] — 已有 Service 列表
- [[reGlasses 跨芯片指令转发]] — 命令如何路由
- [[BLE GATT Service]] — C1 Device Control 接收这些命令
