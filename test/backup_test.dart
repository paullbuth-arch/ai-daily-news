  ...
  group('BackupService 导出/导入', () {
    test('导出生成 zip 含 manifest/data/images', () async {
      ...
      await st.addDevice(Device(
        id: 'd1', serial: 'F1', model: 'iPad Air 5', capacity: '64G',
        color: '银', network: 'WiFi', condition: '95新', purchaseCost: 235000,
      ...
      final zipPath = await BackupService.export(docDir: docDir, storage: st, outDir: outDir);
    ...
    test('导入后数据与图片还原、imagePath 重写为新 docDir', () async {
      ...
      await st1.load();
      await st1.addDevice(Device(
        id: 'd1', serial: 'F1', model: 'iPad Air 5', capacity: '64G',
        color: '银', network: 'WiFi', condition: '95新', purchaseCost: 235000,
        purchaseDate: '2026-06-01', createdAt: '2026-06-01', status: 'sold',
        sellPrice: 290000, sellDate: '2026-06-10',
      ...
      final outDir = '${tmp.path}/out';
      await Directory(outDir).create();
      final zip = await BackupService.export(docDir: src, storage: st1, outDir: outDir);
      ...
      await st2.load();
      final summary = await BackupService.import(zipPath: zip, docDir: dst, storage: st2);
    ...
    test('导入前自动备份生成 .bak', () async {
      ...
      // 先写入一些数据
      await st.addDevice(Device(
        id: 'x1', serial: 'X1', model: 'iPad 9', capacity: '64G',
        color: '银', network: 'WiFi', condition: '95新', purchaseCost: 100000,
        purchaseDate: '2026-06-01', createdAt: '2026-06-01',
      ));
      final outDir = '${tmp.path}/o2';
      await Directory(outDir).create();
      final bak = await BackupService.backupCurrent(docDir: docDir, outDir: outDir);
