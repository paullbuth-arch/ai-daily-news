...
int _scoreModel(String id) {
  ...
  // 排除非对话模型
  if (lower.contains('embedding') || lower.contains('moderation') || lower.contains('tts') || lower.contains('whisper') || lower.contains('dall') || lower.contains('image')) {
    return -100;
  }

  // 排除已废弃/下线的
  if (lower.contains('deprecated') || lower.contains('legacy') || lower.contains('old')) {
    return -50;
  }

  // 轻量版/廉价版 → 低分
  if (lower.contains('mini') || lower.contains('lite') || lower.contains('small') || lower.contains('tiny') || lower.contains('nano') || lower.contains('fast')) {
    score -= 30;
  }
...
Future<List<ModelInfo>?> fetchModels(String modelsUrl, String apiKey) async {
  try {
    ...
    for (final m in modelsList) {
      final id = (m is Map ? (m['id'] as String? ?? m['model'] as String? ?? '') : m.toString());
...
class AiService {
  ...
  static Future<Map<String, dynamic>> _callOpenAI({
    ...
    try {
      ...
      if (resp.statusCode != 200) {
        return {'error': 'AI调用失败(${resp.statusCode})：${raw.substring(0, raw.length > 200 ? 200 : raw.length)}'};
      }
  ...
  static Future<String> chat(String systemPrompt, String userPrompt, {int maxTokens = 4096}) async {
  ...
  static Future<String> chatWithImage(String systemPrompt, String imageBase64, String textPrompt, {int maxTokens = 4096}) async {
    final result = await _callOpenAI(
      systemPrompt: systemPrompt,
      messages: [
        {
  ...
  static Future<Map<String, String>> recognizeAboutThisDevice(String imageBase64) async {
    final sys = '你是苹果设备信息识别专家。用户会上传一张iPad"关于本机"页面的截图。'
    ...
    final result = await chatWithImage(sys, imageBase64, '请识别这张iPad关于本机截图中的信息。', maxTokens: 2048);
    try {
      ...
      return {
        'serial': serialMatch?.group(0) ?? '未知', 'model': '未知', 'capacity': '未知',
        'color': '未知', 'network': '未知', 'batteryHealth': '未知', 'cycleCount': '未知', '_raw': result,
      };
    }
  }

  static Future<String> priceAdvice({
    required String model, required String capacity, required String color,
    required String network, required String condition, required int batteryHealth,
    required int purchaseCost, required int stockDays,
  }) async {
    final sys = '你是二手iPad定价专家。根据设备型号、容量、成色、电池健康、采购成本、库存周转天数，给出定价建议。'
        '输出格式：建议售价¥X，可接受最低价¥Y，建议采购上限价¥Z，定价理由（一句话）。金额单位元。';
    return chat(sys, '设备：$model $capacity $color $network，成色$condition，电池健康度$batteryHealth%，'
        '采购成本${(purchaseCost / 100).toStringAsFixed(0)}元，已在库$stockDays天。请给出定价建议。', maxTokens: 2048);
  }

  static Future<String> dailyReport({
    required int gmv, required int grossProfit, required int orderCount,
    required int inStock, required int stagnant, required int capital,
    required List<String> stagnantModels,
  }) async {
    final sys = '你是二手iPad生意经营顾问。用结构化格式输出今日经营日报。'
        '严格用以下固定格式（emoji标记不能少，每条不超过20字）：\n'
        '【亮点】\n• xxx（今日表现好的地方，不超过3条）\n\n'
        '【待关注】\n• xxx（需要留意的问题，不超过3条）\n\n'
        '【明日建议】\n• xxx（具体可执行的行动，不超过3条）\n\n'
        '不要多余的文字。';
    return chat(sys, '今日GMV ${(gmv / 100).toStringAsFixed(0)}元，毛利${(grossProfit / 100).toStringAsFixed(0)}元'
        ...
        '滞销机型：${stagnantModels.isEmpty ? "无" : stagnantModels.join("、")}。请生成日报。', maxTokens: 2048);
  }

  static Future<String> customerService(String question) async {
    return chat('你是二手iPad客服助手，回答买家关于成色、电池、价格、保修等问题。态度热情专业，回答简洁。', question, maxTokens: 1024);
  }

  static Future<String> purchaseAdvice(String recentSalesSummary) async {
    return chat('你是二手iPad采购顾问。根据近期销售情况，给出补货建议：该补哪些型号、补多少、采购上限价多少。',
        '近期销售：$recentSalesSummary\n请给出本周采购建议。', maxTokens: 2048);
  }

  static Future<String> generateDescription({
    required String model, required String capacity, required String color,
    required String network, required String condition, required int batteryHealth,
    required int cycleCount, required bool idLockClean, String accessories = '裸机',
  }) async {
    final sys = '你是闲鱼二手iPad商品文案专家。根据给定的设备准确信息，写一段真实、专业、有吸引力的商品描述。'
        '要求：①突出成色、电池健康、ID锁状态等买家最关心的点；②语言自然不浮夸，不夸大；③包含配置亮点和适用场景；'
        '④100-180字，纯文本不带emoji和特殊符号、不带价格；⑤不要分点编号，写成流畅的一段话。';
    final r = await chat(sys, '请为以下设备写商品描述：\n型号：$model\n容量：$capacity\n颜色：$color\n网络：$network\n成色：$condition\n'
        '电池健康度：$batteryHealth%\n充电循环：$cycleCount次\nID锁：${idLockClean ? "无锁（干净）" : "有锁"}\n配件：$accessories', maxTokens: 2048);
  ...
  static Future<String> purchaseDecision({
    required String model, required int purchaseCost, required int quantity,
    required Map<String, dynamic> analysis,
    Map<String, dynamic>? marketPrice, List<Map<String, dynamic>>? marketHistory,
  }) async {
    final sys = '你是二手iPad采购决策顾问。综合"今日市场行情、历史销售数据、拟采购成本"判断"是否建议按此成本采购该数量"。'
        '输出格式：\n【结论】建议收 / 谨慎收 / 不建议\n【理由】不超过3条\n【风险】1条\n中文，简洁，总字数≤260。';
    final a = analysis;
    final suppliersStr = (a['suppliers'] as List)
        .map((s) => '${s['channel']}(${s['count']}台,均利${((s['profit'] as int) / 100).toStringAsFixed(0)}元)')
        .join('、');
    ...
    if (marketPrice != null) {
      ...
      marketStr = '今日批发价${mp}元（${marketPrice['date']}），采购成本${(purchaseCost / 100).toStringAsFixed(0)}元，差价${diff > 0 ? "+" : ""}$diff元（毛利率$margin%）';
      if (marketHistory != null && marketHistory.length >= 2) {
        ...
        final pct = oldest > 0 ? (change / oldest * 100).toStringAsFixed(1) : '0';
        trendStr = '\n- 行情趋势：近${marketHistory.length}天从${oldest}元→${newest}元（${change > 0 ? "↑" : change < 0 ? "↓" : "→"}${pct}%）';
      }
    }
    return chat(sys, '型号：$model\n拟采购：${(purchaseCost / 100).toStringAsFixed(0)}元/台 × $quantity 台\n'
