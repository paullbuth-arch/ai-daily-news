...
I2S 解决的是"芯片间传输音频数据"的问题，就像一条音频流水线传送带，节拍器（位时钟）决定传多快，分界线（左右声道时钟）区分左声道和右声道。I2S 传的是标准的 PCM 数字音频，接收方可以直接用，不需要额外转换。初学者先记住：I2S 有三根核心线（位时钟、左右声道时钟、数据），位时钟的频率等于声道数乘以位宽乘以采样率。
