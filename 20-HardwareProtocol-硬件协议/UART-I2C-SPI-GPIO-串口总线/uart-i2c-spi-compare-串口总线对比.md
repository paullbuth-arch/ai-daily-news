...
这三种总线不是谁更快就用谁，选型取决于三个约束的交叉点：电气结构（推挽还是开漏决定了一对多能力和速度上限）、时钟模型（异步还是同步决定了误差容忍度）、工程成本（线数、GPIO 开销、布线复杂度）。初学者先记住：UART 点对点、I2C 多设备共享两根线、SPI 高速但每设备多一根线，没有一种总线在所有场景下都最优。
