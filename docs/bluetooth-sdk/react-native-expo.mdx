  ...
  return (
    <View>
      ...
      <Button
        ...
        onPress={() => mentra.setGalleryModeEnabled(!mentra.sdk.galleryMode.enabled)}
      />
...
Use `useMentraBluetooth()` as the React status API for connection, battery, Wi-Fi, hotspot, scan, and SDK runtime state. `mentra.sdk.galleryMode.enabled` reflects the desired gallery-mode setting sent to the glasses, and `mentra.sdk.galleryMode.applying` is true while `setGalleryModeEnabled(...)` is in flight.
