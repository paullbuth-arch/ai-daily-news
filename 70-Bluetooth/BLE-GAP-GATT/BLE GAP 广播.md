...
广播是 BLE 通信的**第一步**——手机必须先"发现"你的眼镜，才能建立连接、才能用 [[BLE GATT]] 交换数据。广播没配对，后面全白搭。
...
广播只是"举招牌"，手机决定连接后，进入 [[BLE GATT]] 阶段。连接参数：
...
广播搞懂了 → 去看 [[BLE GATT]] — 连接建立后的数据交互。
或者 → 去看 [[BLE SMP 配对]] — 安全配对和加密。

## 关联概念

- [[BLE GATT]] — 连接建立后的数据交互层
- [[BLE SMP 配对]] — 安全配对和链路加密
- [[BLE GATT Service]] — 广播中通告的 Service UUID
- [[reGlasses 协议架构]] — BLE 在系统中的位置
- [[WQ7036AX 芯片]] — BLE 射频硬件
