...
> ⚠️ **已弃用**：STTP 因项目只需要传语音指令，其 256 通道的复杂复用机制过于庞大，已被更轻量的 [[uart-basics-UART基础 (app_uart_cmd)]] 替代。当前协议不使用通道机制，而是用 5 种固定命令类型区分业务。本笔记保留作为历史参考。
...
STTP 在 SDK 中通过 [[ext-trans-Ext-Trans框架 集成：]]
...
- [[ext-trans-Ext-Trans框架]] — STTP 如何通过 SDK 框架集成
- [[reglasses-cross-chip-reGlasses跨芯片转发]] — 通道在项目中的实际使用
