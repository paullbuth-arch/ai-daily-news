...
WQ7036AX 是手机和 V881 之间的**翻译官**：手机用 [[wq-audio-protocol-WQ-Audio-Protocol]] 说 BLE 语，V881 用 [[uart-basics-UART基础]] 说 UART 语。WQ7036AX 在中间**听懂 BLE 语 → 翻译成 UART 语 → 传给 V881**。但有些指令不需要翻译——比如语音控制，WQ7036AX 自己就能做。

## 30 秒先看懂

reGlasses 用两颗芯片（WQ7036AX 和 V881）分工协作，但手机只连接 WQ7036AX 的蓝牙。如果手机要控制摄像头（摄像头在 V881 上），WQ7036AX 必须把指令从 BLE 协议翻译成 UART 协议转发给 V881。就像一台翻译机，听懂一种语言再转成另一种语言传给对方。初学者先记住：硬件在谁身上，谁来做——摄像头在 V881 上就转发，麦克风在 WQ7036AX 上就本地处理。

## 学完以后应该能做什么

1. 能说出"硬件在谁身上谁来做"的路由原则。
2. 能列出至少 3 个需要转发给 V881 的命令和 2 个本地处理的命令。
3. 能画出"开始录制"的完整时序图（手机→WQ→V881→WQ→手机）。
4. 能在 SDK 中找到 `app_trans.c` 并说出消息分发入口。
...
- 知道 UART 的基本通信原理；可先看 [[uart-basics-UART基础]]。
- 了解 reGlasses 的整体硬件拓扑；可先看 [[reglasses-architecture-reGlasses协议架构]]。
- 如果要理解命令的完整路由路径，需要 [[dataflow-cmd-to-v881-手机指令到V881]]。

## 术语先讲清楚

| 术语 | 英文 | 在跨芯片转发中具体指什么 |
|---|---|---|
| 本地执行 | local execution | WQ7036AX 收到指令后自己处理，不需要转发给 V881。如语音采集、Opus 编码 |
| 跨芯片转发 | cross-chip forwarding | WQ7036AX 收到指令后通过 UART 转发给 V881 执行。如录制、拍照、TOF 控制 |
| UART 命令协议 | UART command protocol | WQ↔V881 之间使用的轻量级帧协议，5 种固定命令，XOR 校验 |
| 翻译官模式 | translator mode | WQ7036AX 在 BLE 和 UART 两种协议之间翻译转发的角色 |
...
**通过标准**：能说出每个命令的判断依据（硬件在谁身上）。

### 练习二：画时序图

画出"手机查询设备状态"的完整时序图，包括 WQ 填本地状态（电量/BLE 状态）、通过 UART 问 V881（WiFi/存储状态）、合并结果返回手机。

**通过标准**：能标注每一步的协议转换点（BLE→UART→BLE）。

## 自测题

1. **WQ7036AX 收到 CTRL_RECORD_START 后是本地执行还是转发 V881？**
   - 转发 V881。因为摄像头硬件连接在 V881 上，WQ7036AX 没有摄像头驱动。

2. **CTRL_VOICE_REQ 谁来做？为什么？**
   - WQ7036AX 本地做。因为麦克风（PDM）直接连在 WQ7036AX 上，音频采集和 Opus 编码都由 WQ7036AX 的 DSP 完成，不需要 V881 参与。

3. **STATUS_QUERY 为什么需要 WQ 和 V881 都参与？**
   - WQ 填本地状态（电量、BLE 连接），V881 填远程状态（WiFi 连接、存储空间），两者合并后返回手机。

## 关联概念

- [[reglasses-architecture-reGlasses协议架构]] — 整体拓扑
- [[uart-basics-UART基础]] — UART 侧帧格式
- [[dataflow-cmd-to-v881-手机指令到V881]] — 完整追踪
- [[frame-compare-帧协议对比]] — 两种帧的差异
